...
const ejecutarComando = async () => {
    ...
    if (normalizar(comando.value) === normalizar(consultaCorrecta.value)) {
        ...
        try {
            ...
            await fetch('https://command-sql-back.onrender.com:8000/logros/logrousuario', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${token}`
...
onMounted(async () => {
    try {
        ...
        // 📘 Obtener misión
        const resMision = await fetch('https://command-sql-back.onrender.com:8000/misiones/1', {
            headers: { Authorization: `Bearer ${token}` }
        });
